...
impl Request {
    ...
    pub fn from_stream(stream: &mut impl Read, limits: &Limits) -> MzaniResult<Self> {
...
pub(crate) fn read_header_block(stream: &mut impl Read, max_header_bytes: usize) -> MzaniResult<(Bytes, Bytes)> {
...
mod tests {
    ...
    fn read_headers_reads_until_block_end() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (raw_head, body_prefix) = read_header_block(&mut cursor, limits.max_header_bytes)?;
    ...
    #[test]
    fn parse_head_rejects_garbage_corpus() {
        let seeds: &[&[u8]] = &[b"", b"\r\n\r\n", &[0xff, 0xfe, 0xfd]];
        for raw in seeds {
            assert!(parse_head(raw).is_err(), "expected parse error for {raw:?}");
        }
    }
    ...
    fn rejects_body_over_limit() {
        ...
        let payload = b"POST / HTTP/1.1\r\nContent-Length: 10\r\n\r\n1234567890";
        let mut cursor = Cursor::new(payload.to_vec());
        let result = Request::from_stream(&mut cursor, &limits);
        assert!(matches!(result, Err(MzaniError::BodyTooLarge)));
    }
}
